...
class DelSuc : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_del_suc)
        ...
        Handler(Looper.getMainLooper()).postDelayed({ // в данном месте кода объявляется класс "Handler" который предназначен для взаимодействия между формами. Обращатся кметоду 'получить основной цикл' и также к методу таймера
            val intent = Intent(this,DelSuc2::class.java) // тут объявляеться локальная переменная "intent" и присваивается к классу типа "Intent", после в скобках класса происходит сама логика перехода.
